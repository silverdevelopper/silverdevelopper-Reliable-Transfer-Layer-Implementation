...
void A_output(struct msg message)
{
    ...
    struct pkt pck;
    pck.seqnum = A.seq;
    memmove(pck.payload, message.data, 20);
    pck.checksum = compute_check_sum(pck);
    A.last_packet = pck;
    A.state = WAIT_ACK;
    tolayer3(0, pck);
...
void A_input(struct pkt packet)
{
  ...
  if (packet.checksum != compute_check_sum(packet))
  {
...
void B_timerinterrupt()
{
  ...
  printf(" Not implemented. B does not have a timer.. \n");
